<div>
    <x-card>
        <x-slot:header>
            <div class="flex flex-wrap items-center justify-between -mt-2 -ml-4 sm:flex-nowrap">
                ...
                <div class="flex-shrink-0 mt-2 ml-4">
                    <button wire:click.prevent="manageAddresses" type="button" class="btn btn-link">
                        {{ __('Manage') }}
                    </button>
                </div>
            </div>
        ...
        <x-slot:content class="-mt-5">
            ...
            @endunless
            <div class="mt-2">
                <button wire:click.prevent="create" type="button" class="btn btn-link">
                    {{ __('Add new address') }}
                </button>
            </div>
        </x-slot:content>
    ...
    <x-modal-dialog wire:model.defer="showAddressForm">
        ...
        <x-slot:content>
            <fieldset wire:target="save" wire:loading.attr="disabled" class="grid grid-cols-6 gap-6 mt-5">
                ...
                <div class="col-span-6 md:col-span-3 lg:col-span-4">
                    ...
                    <x-input wire:model.defer="address.name" type="text" id="name"
                                ...
                                <ul x-show="open" x-on:click.away="open = false"
                                    ...
                                    <li x-on:click="$wire.selectCountry('{{ $country->iso2 }}'); open = false;"
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span class="block ml-3 font-normal truncate">{{ $country->name }}</span>
                                        </div>
                                    </li>
                                    @endforeach
                                </ul>
        ...
        <x-slot:footer>
            <button wire:click.prevent="save" wire:target="save" wire:loading.attr="disabled" type="submit"
                class="w-full btn btn-primary sm:ml-3 sm:w-auto">
                {{ __('Save') }}
            </button>
            <button wire:click="$set('showAddressForm', false)" wire:target="save" wire:loading.attr="disabled"
                type="button" class="w-full mt-3 btn btn-invisible sm:mt-0 sm:w-auto">
                {{ __('Cancel') }}
            </button>
        </x-slot:footer>
    </x-modal-dialog>
        ...
        <x-slot:content>
            <ul class="divide-y divide-slate-200 dark:divide-slate-200/10">
                ...
                <li class="py-4">
                    ...
                    <span class="block mb-3 text-xs font-medium uppercase text-slate-900 dark:text-slate-200">
                        {{ __('Default address') }}
                    </span>
                    @endif
                    <address class="text-sm not-italic">
                    ...
                    <div class="flex items-center justify-between mt-3">
                        <button wire:click.prevent="edit('{{ $customerAddress->id }}')" class="btn btn-link">
                            {{ __('Edit address') }}
                        </button>
                        @unless($customerAddress->is_default)
                        <button wire:target="setDefaultAddress('{{ $customerAddress->id }}')"
