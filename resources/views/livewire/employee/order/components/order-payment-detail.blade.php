<div>
    <x-card>
        ...
        <x-slot:content class="-mx-4 -my-5 sm:-mx-6">
            <dl @class(['border-t border-slate-200 divide-y divide-slate-100 dark:border-slate-200/20
                dark:divide-slate-200/5', '-mb-5'=> $order->payment_status === \App\Enums\PaymentStatus::PAID])>
                <div class="p-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="flex mt-1 text-sm leading-6 text-slate-700 sm:col-span-2 sm:mt-0 dark:text-slate-400">
                        <div class="flex-grow">
                            {{ trans_choice(':count item|:count items', $this->total_order_items_quantity) }}
                        </div>
                        <div class="flex-shrink-0 ml-4 text-right tabular-nums">
                    ...
                    <dd class="flex mt-1 text-sm leading-6 text-slate-700 sm:col-span-2 sm:mt-0 dark:text-slate-400">
                        ...
                        <div class="flex-shrink-0 ml-4 text-right tabular-nums">
                            <ul>
                                @foreach($order->orderDiscounts as $discount)
                                <li>
                                    <x-money :amount="$discount->discounted_amount"
                ...
                <div class="flex items-center justify-between p-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd
                        class="text-sm font-medium text-slate-900 tabular-nums sm:col-span-2 sm:text-right dark:text-slate-200">
                ...
                <div class="p-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd
                        class="mt-1 text-sm font-medium text-slate-900 sm:mt-0 sm:col-span-2 sm:text-right tabular-nums dark:text-slate-200">
                        <x-money :amount="$order->total_paid - $order->total_refunded"
                            :currency="config('app.currency')" />
                    </dd>
                </div>
                @endif
        ...
        @if(! $order->paymentMethod->is_third_party && $order->payment_status !== \App\Enums\PaymentStatus::PAID && !
        $order->refunds->count())
        <x-slot:footer class="bg-slate-50 dark:bg-slate-800/75">
            <div class="flex justify-end">
                <button wire:click="confirmMarkingPaymentAsPaid" wire:target="confirmingMarkingAsPaid"
    ...
    <x-modal-alert wire:model="confirmingMarkingAsPaid">
        ...
        <x-slot name="footer">
            <button wire:click="markAsPaid" wire:target="markAsPaid" wire:loading.attr="disabled"
                class="w-full btn btn-primary sm:ml-3 sm:w-auto">
                {{ __('Mark as paid') }}
            </button>
            <button x-on:click="show = false" class="w-full mt-3 btn btn-invisible sm:mt-0 sm:w-auto">
                {{ __('Cancel') }}
            </button>
        </x-slot>
    </x-modal-alert>
